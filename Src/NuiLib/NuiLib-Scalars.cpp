...
AngleScalar *NuiLib::angleP(IVector *vector1, IVector *vector2) {
	AngleScalar *scalar = ExtensionFactory()->Make<AngleScalar>(vector1->GetName() + "." + vector2->GetName());
	scalar->SetVector1(vector1);
	scalar->SetVector2(vector2);
	scalar->Changed(vector1);
	return scalar;
...
Scalar NuiLib::angle(const Vector &vector1, const Vector &vector2) {
	return angleP(vector1._p, vector2._p);
}
